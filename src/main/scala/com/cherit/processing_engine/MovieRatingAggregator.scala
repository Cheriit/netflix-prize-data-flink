...
import com.cherit.models.{MovieRatingAccumulator, MovieRatingAggregated, MovieRating}
...
class MovieRatingAggregator extends AggregateFunction[MovieRating, MovieRatingAccumulator, MovieRatingAggregated] {
  ...
  override def getResult(acc: MovieRatingAccumulator): MovieRatingAggregated = MovieRatingAggregated(acc.movieId, acc.ratingCount, acc.ratingSum, acc.ratingUsers.count())
